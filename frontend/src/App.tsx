...
function PersonEditor({ form, setForm, onSave, onCancel }: { form: Partial<Person>; setForm: (form: Partial<Person>) => void; onSave: () => void; onCancel: () => void }) { const update = (key: keyof Person, value: string | number | null) => setForm({ ...form, [key]: value }); return <div className="modal-body"><div className="edit-grid"><label>이름<input value={form.name ?? ''} onChange={(event) => update('name', event.target.value)} /></label><label>계급<input value={form.rank ?? ''} onChange={(event) => update('rank', event.target.value)} /></label><label>군종<input value={form.branch ?? ''} onChange={(event) => update('branch', event.target.value)} /></label><label>소속부대<input value={form.unit ?? ''} onChange={(event) => update('unit', event.target.value)} /></label><label>특기<input value={form.specialty ?? ''} onChange={(event) => update('specialty', event.target.value)} /></label><label>직책<input value={form.position ?? ''} onChange={(event) => update('position', event.target.value)} /></label><label>복무연도<input type="number" min="0" max="8" value={form.service_year ?? ''} onChange={(event) => update('service_year', Number(event.target.value))} /></label><label>상태<select value={form.status ?? 'active'} onChange={(event) => update('status', event.target.value)}><option value="active">복무 중</option><option value="on_leave">휴가 중</option></select></label></div><div className="form-actions"><button className="button secondary" onClick={onCancel}>취소</button><button className="button primary" onClick={onSave}>저장</button></div></div> }
